...
void print_statistics(int rel)
{
	...
	read_systime(&now);
	if (rel) {
		double relfactor = 100 / timediff(init.total,   now.total);
		printf("%05d %7.2f%% %7.2f%% %7.2f%% %7.2f%% %7.2f%% %7.2f%% %7.2f%% %7.2f%% %8.3f\n",
				counter,
				relfactor * timediff(init.user,    now.user),
				relfactor * timediff(init.nice,    now.nice),
				relfactor * timediff(init.system,  now.system),
				relfactor * timediff(init.idle,    now.idle),
				relfactor * timediff(init.iowait,  now.iowait),
				relfactor * timediff(init.irq,     now.irq),
				relfactor * timediff(init.softirq, now.softirq),
				relfactor * timediff(init.steal,   now.steal),
...
void print_statistics_verbose(int rel)
{
	read_systime(&now);
	fprintf(stderr, "\n");
	if (rel) {
		double relfactor = 100 / timediff(init.total,   now.total);
		fprintf(stderr, "user    %8.3f %%\n", relfactor * timediff(init.user,    now.user));
		fprintf(stderr, "nice    %8.3f %%\n", relfactor * timediff(init.nice,    now.nice));
		fprintf(stderr, "system  %8.3f %%\n", relfactor * timediff(init.system,  now.system));
		fprintf(stderr, "idle    %8.3f %%\n", relfactor * timediff(init.idle,    now.idle));
		fprintf(stderr, "iowait  %8.3f %%\n", relfactor * timediff(init.iowait,  now.iowait));
		fprintf(stderr, "irq     %8.3f %%\n", relfactor * timediff(init.irq,     now.irq));
		fprintf(stderr, "softirq %8.3f %%\n", relfactor * timediff(init.softirq, now.softirq));
		fprintf(stderr, "steal   %8.3f %%\n", relfactor * timediff(init.steal,   now.steal));
	} else {
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "sr")) != -1)
	{
		...
		if (opt == 'r') {
			...
			fprintf(stderr, "Usage: %s [ -r ] [ -s ] [ command [ arg.. ] ]\n", argv[0]);
			return 1;
		}
	}
	...
	if (optind >= argc) {
		...
		int status;
		wait(&status);
		print_statistics_verbose(mode_rel);
